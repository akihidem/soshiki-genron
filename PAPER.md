...
- claude 監督は無料枠（`claude-cli-run.py`＝対話TUI）経由。決定的部分は **61 tests 緑**。
...
3. 膜は3方向から崩れる: **監督が能力差で崩れる**（recall）／**強い監督が過剰flag する**（precision・膜が消える）／**競争が薄くする**（race）。
4. **能力そのものが膜＋仕様に上限づけられる**（F7：安全な p\* は仕様+検証次第）。
→ 「AIネイティブの組織は組織でなくデータフロー＋薄い膜」だが、**薄い膜の設計と維持が最難**で、そこが安全の急所。
