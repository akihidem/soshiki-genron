...
| **④ 補完能力 mesh** | 縁でだけ・**異なる能力**を組む（ブランド混合でない） | decorr: 能力差で誤り脱相関→union>単独(+0.1)／実 SWE-bench N=24: opus×codex も*相互*相補で union>best だが **gain +0.04 と小**＝**燃料は能力相補性**（弱+強>強+強・ブランド差でない・§5） |
...
> 一般教訓: AIネイティブな組織・制度行動の実証は、**安全訓練・フレーミング・harness 産物・測定器の交絡・自己評価のモデル依存・測定基準自身の誤り・係数の非同定性・agentic-framing・LLM-judge の merge/長さ/自己びいき・実タスクの環境/採点器/出力形式 artifact**に晒される。素の異常値・単一条件は誤導する（cross-vendor 合成 +0.83 は controlled 比較=同ベンダ vs 異ベンダ で −0.08 に消えた＝交絡だった／実 SWE-bench 初回 all-zero は3層 harness artifact だった）。measurement-first（測定という外部錨）・controlled 対照・**陽性対照(known-correct→1)** が効いた。
