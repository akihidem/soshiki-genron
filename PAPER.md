...
- claude 監督・プレイヤー・worker は無料枠（`claude-cli-run.py`＝対話TUI・transient hang に retry+fail-fast+再開キャッシュで耐性）経由。決定的部分は **78 tests 緑**。
...
- **次（大きな新フェーズ）**: **実質的能力差のある agent 群での market 検定**（小型モデル vs frontier ── frontier 同士では古典問題で勾配が出ず＝均質有能群では flat が勝つと外部 gold でも確定。市場便益は*大きな*能力差が前提という仮説の検証）／統計強化（trials>1）／他係数の実測較正（通信コスト・Goodhart の code-overfitting）／フレーミング感応の一般化測定／F7 整合の精緻化。組織シミュレータは3構造×実行正しさ＋**異種 market（self-test 版・外部 gold 版）まで接地済み**（最小実装・n=3。異種便益は frontier 同士では未顕在＝顕在化の条件は「大きな能力差」と判明）。
