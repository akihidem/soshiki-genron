...
- **レースの実証・複製・機構**（`experiments/race_game.py`）: 理由づけを読むと機構が見えた。**opus は競争を*合理的に*プレイし baseline でも liability でも S=0**。その理由が核心 ── 「winner-take-all なので安全＝確実に負け（ゼロ）、しかも liability は*自分が勝って*破局を*起こした*時しか払わない → 賭けて勝ちにいく」と**正しく推論**。＝**勝者責任の liability は winner-take-all レースを抑止しない**（解析で liability が最効率だったのを*構造で修正*）。一方 **mandate（全員に床0.7）は機能**: opus も「全員が縛られ defection 不可 → 0.7 が支配戦略」と合理的に遵守 ── **mandate はレース*自体を消す*から効く**。haiku/sonnet は多くシナリオを**拒否**（安全訓練・sonnet は harness の完了 sentinel まで injection とフラグ）＝プレイせず、cross-model 比較は交絡。→ **AIレース統治は「誘因設計（liability）」より「レースを消すハードルール（mandate/標準）」に寄せるべき** ── 最も合理的なエージェントがその理由を自ら示した。N小・拒否交絡あり。**機構が「数値の walk-back」を*説明*した** —— measurement-first の核心。
