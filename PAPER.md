...
| **④ 補完能力 mesh** | 縁でだけ・**異なる能力**を組む（ブランド混合でない） | decorr: 能力差で誤り脱相関→union>単独／xvs: 同水準異ベンダ opus×codex は多様性 gain ≈0 ＝**燃料は能力相補性** |
...
> 一般教訓: AIネイティブな組織・制度行動の実証は、**安全訓練・フレーミング・harness 産物・測定器の交絡・自己評価のモデル依存・測定基準自身の誤り・係数の非同定性・agentic-framing・LLM-judge の merge/長さ/自己びいき**に晒される。素の異常値・単一条件は誤導する（cross-vendor 合成 +0.83 は controlled 比較=同ベンダ vs 異ベンダ で −0.08 に消えた＝交絡だった）。measurement-first（測定という外部錨）と controlled 対照が効いた。
