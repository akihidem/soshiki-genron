...
- **閾値の実測較正**（`market_external.py --calibrate`・gemma trials=3）: 支配定理の唯一 noisy な量＝弱ティアの完全解率 p を gemma で固めた。**p_weak: 0.667(trials1) → 0.889(trials3)**。per-task の solve_rate(1.0 到達率): clamp/leap/revwords/intervals/wildcard は **1.0（常に解く）**、**roman だけ 0.333（構造的に難）**。→ **gap 実証の "leap=0.0" は*ノイズ*（trials=3 で常に 1.0）・roman だけが真の能力差**（§9-1「n=1 を信用しない」の再演）。支配地図: gemma→{haiku, sonnet, opus} **全ペアが支配領域 p>w/s に入る**（market コスト 0.311/0.534/1.87）＝**大能力差での市場支配は頑健**（較正でむしろ market コスト 0.533→0.311 に低下＝支配が*強まる*）。**この 3 ペアは標本誤差の床（[`model/GAP.md`](model/GAP.md)）を通しても全て支配**（最も厳しい →haiku で要 p̂≥0.667 に対し実測 p̂=0.889）＝**本論の主結論は床の後も立つ**（崩れたのは `--map` の周縁セル 3 件と、境界に寄るための*計画*）。[`MARKET_CALIB.md`](experiments/MARKET_CALIB.md)
- **支配地図（複数ローカルモデル）**（`market_external.py --map` → `--regate`）: gemma **e2b(5.1B)/latest(8B)/chat(5.1B)** の p_weak を測り (p, w/s) 平面に置いた。p_weak: e2b 0.83 / latest 0.5 / chat 0.42。**当初は 9 ペア全てに ✓（支配）を刻んだが、これは点推定を標本誤差に通さない fail-open だった**（`dominates = (p̂ > w/s)`）。**標本誤差の床**（[`model/GAP.md`](model/GAP.md)）を通すと、n=6 では **9 ペア中 3 件が「判定不能」**（latest→haiku・chat→haiku・chat→sonnet）── 観測は「真に境界上（市場の利得ゼロ）」という帰無と**両立する**。実際、真に境界上のモデルに対しても現行手続きは **34.5%** の確率で ✓ を立てる（床を通せば 0.017 ≤ α）。余裕の大きい残り 6 件は床を越えて支配。
  - latest/chat の低 p には **trial-0 の cold-start 切り詰め artifact** 仮説がある（trial-0 が len~60 で全タスク失敗・trial-1 は完全正答）。だがこれは*事後解釈*であり、「悪い試行を捨てれば真 p≈1.0」と読むのは §9-1 の逆走である。正しい応答は再解釈でなく **再測定**（warm-up ＋ タスク数増）。
  - → **「境界(p≈0.2) はこれらより弱いモデル/難タスクで測ればよい」は誤りだった。** n=6 では下側の枝が空で、**p̂=0/6（全問不正解）ですら「非支配」と言えない**（P(X=0|p=0.2)=0.262 > α）＝**支配の主張は n=6 では原理的に反証不能**。弱いモデルを足すほど判定不能帯の奥へ入るだけである。境界に寄る唯一の道は **n を増やすこと**（反証可能性の発生に **n≥14**・境界から δ=0.05 の分解能に **n=224**）。[`MARKET_MAP.md`](experiments/MARKET_MAP.md)／[`model/GAP.md`](model/GAP.md)
...
12. **点推定の符号比較を「判定」に使うな ── *境界に寄る前に分解能を計算しろ*** — 支配定理 p\*=w/s は**真の p** の話なのに、実測は `dominates = (p̂ > w/s)` と**推定量の符号をそのまま二値判定**していた。真に境界上（利得ゼロ）のモデルに対しても、この手続きは n=6 で **34.5%** の確率で「支配」と刻む（床を通せば 0.017 ≤ α）。**閾値のある理論を実測で追うとき、境界の近傍こそ最も判定が壊れる** ── そして「境界に寄せて実測する」とは、定義上その近傍へ行くことである。さらに悪いことに、**n=6 では下側の枝が空**で p̂=0/6 ですら「非支配」と言えず、**支配の主張は原理的に反証不能**だった＝ロードマップに書いた次の実験（「より弱いモデルで境界に寄せる」）は**走らせる前から空振りが確定していた**。分散は Var(p̂) = [Var_task(p_i) + E[p_i(1−p_i)]/t]/n で、**タスク間分散は trials では消えない**（実測の per-task が 0/1 に張り付く＝能力差が*構造的*な極では trials の寄与は厳密にゼロ）。→ **効くのは n であって trials ではない**（§9-11 が mesh 軸で出した結論に、市場軸から独立に到達）。**実験を設計する前に「その n で境界からどれだけ離れていれば判定できるか」を計算せよ**（反証可能性の発生に n≥14・δ=0.05 の分解能に n=224）。[`model/GAP.md`](model/GAP.md)
