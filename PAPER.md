...
- claude 監督・プレイヤー・worker は無料枠（`claude-cli-run.py`＝対話TUI・transient hang に retry+fail-fast+再開キャッシュで耐性）経由。決定的部分は **75 tests 緑**。
...
> 一般教訓: AIネイティブな組織・制度行動の実証は、**安全訓練・フレーミング・harness 産物・測定器の交絡・自己評価のモデル依存**に晒される。単発/単一フレーミング/素の異常値は誤導する。本研究が3度自己修正したのは、measurement-first（測定という外部錨）が効いた証拠。

## 10. 寄与と次
- **寄与**: 「AIにとって組織とは何か」を構造でなく機能から再導出し、各主張を反証可能な計測に落とし、中心パラメタを実測較正し、制度的 countermeasure を実証した、再現可能な公開研究。**組織論の AI 版の最初の骨格＋その測り方の教訓**。
- **次（大きな新フェーズ）**: **外部正解スイートでの heterogeneity 再検定**（self-test 交絡を外し・弱モデルが*客観的*に落ちる難タスクで escalation market が単一モデル前線を支配するか）／統計強化（trials>1）／他係数の実測較正（通信コスト・Goodhart の code-overfitting）／フレーミング感応の一般化測定／F7 整合の精緻化。組織シミュレータは3構造×実行正しさ＋**異種 market まで接地済み**（最小実装・n=3。異種便益は本タスク群では未顕在＝顕在化する条件の特定が次）。
