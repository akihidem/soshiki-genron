...
- **構造の実証（F3・最大の gap を埋めた）**（`experiments/org_sim.py`）: 実 LLM エージェント（sonnet）に相互依存タスク（設計→実装→テスト）を **flat（共有黒板）vs hierarchy（管理者が分解・割当・統合）** で組織させ、コスト（コール数）と品質（静的 coverage）を測った。結果: **flat = 3コールで品質 1.0／hierarchy = 5コールで品質 0.67** ── **flat の方が*安く*かつ*品質も高い***。機構も観測: hierarchy の**管理者の統合ステップが関数を 2/3 取りこぼした**（lossy bottleneck）＝解析が仮定した "mediation loss" が実物で出た。→ **解析 F3（flat 安い）＋ tehai A/B を*実エージェント*で支持、さらに品質でも flat 勝ち**。これまで実証は F8（監督）だけ → **構造軸 F3 も実証された**。N=2・静的品質（正しさ未検証）。
...
実証の過程で結論が**2度自己修正**された。その修正自体が、AIの組織・制度行動を計測する上での転移可能な教訓:
...
> 一般教訓: AIネイティブな組織・制度行動の実証は、**安全訓練・フレーミング・harness 産物**に交絡する。単発/単一フレーミングは誤導する。本研究が2度自己修正したのは、measurement-first（測定という外部錨）が効いた証拠。

## 10. 寄与と次
- **寄与**: 「AIにとって組織とは何か」を構造でなく機能から再導出し、各主張を反証可能な計測に落とし、中心パラメタを実測較正し、制度的 countermeasure を実証した、再現可能な公開研究。**組織論の AI 版の最初の骨格＋その測り方の教訓**。
- **次（大きな新フェーズ）**: 完全なエージェント組織の実証基盤／他係数の実測較正（通信コスト・Goodhart の code-overfitting）／フレーミング感応自体の一般化測定／F7 整合の形式の精緻化。
