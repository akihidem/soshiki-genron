...
- **誤りの脱相関と「縁の組織形態」（cross-vendor 創発の糸口）**: 創発の源泉は redundancy(union/best-of)でなく **誤りの脱相関**（異種ベンダの失敗が*別 instance* なら相互検証 mesh が単一を超える＝ML アンサンブル原理を組織へ）。だが実測: **fraction(30ケース)も LeetCode-hard(2D雨水・cherry pickup)も opus/codex は全員正解＝誤りゼロ → 脱相関ゼロ → mesh が exploit する盲点が無い**。→ **cross-vendor mesh は「能力の縁」の組織形態**: ①標準タスク（含 hard）＝frontier 誤らず mesh 無価値（escalation で足る）／②**縁（frontier が誤り始める）＝誤りが脱相関なら mesh が単一超え＝新組織形態**／③縁の外/open-ended＝synthesis は効きうるが客観計測困難。**多様性は*常に*でなく単一モデルが信頼できなくなる所で点火する**（仕事が縁へ動くほど mesh の価値↑）。**実証の限界**: frontier が*両方誤る* checkable task を構築できず（縁に届かない）＝新組織形態の実証は novel/訓練外 task か open-ended 領域を要する（それ自体が困難の所在）。
