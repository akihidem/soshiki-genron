...
> 一般教訓: AIネイティブな組織・制度行動の実証は、**安全訓練・フレーミング・harness 産物・測定器の交絡・自己評価のモデル依存・測定基準自身の誤り・係数の非同定性**に晒される。単発/単一フレーミング/素の異常値は誤導する。本研究が3度自己修正したのは、measurement-first（測定という外部錨）が効いた証拠。
