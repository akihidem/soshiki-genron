...
- claude 監督・プレイヤー・worker は無料枠（`claude-cli-run.py`＝対話TUI・transient hang に retry+fail-fast+再開キャッシュで耐性）、gemma は local ollama 経由。決定的部分は **78 tests 緑**。
...
- **次（大きな新フェーズ）**: 統計強化（trials>1・**能力差の連続スイープ**＝市場が単一モデル前線を支配し始める能力差の閾値の特定）／他係数の実測較正（通信コスト・Goodhart の code-overfitting）／フレーミング感応の一般化測定／F7 整合の精緻化。組織シミュレータは3構造×実行正しさ＋異種 market を**3レジーム（均質 sonnet／frontier 異種／大能力差 gemma×frontier）で接地済み**＝**市場価値は能力差条件付き・大能力差で検証ルーティング市場が Pareto 支配**を実証（最小実装・n小）。
