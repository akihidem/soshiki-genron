...
- claude 監督・プレイヤー・worker は無料枠（`claude-cli-run.py`＝対話TUI・transient hang に retry+fail-fast+再開キャッシュで耐性）、gemma は local ollama 経由。決定的部分は **93 tests 緑**。
...
## 8. 限界・妥当性
- 決定的な**トイモデル**。係数は第一原理的だが*仮定*（**oversight_error・過剰flag・mgr_overhead(通信)・市場閾値 p・Goodhart の向きと大きさ を実測較正済み**；Goodhart 指数の精密値・spec gap の絶対値・通信の token コストはなお未較正）。結論は質的（向き）＋感度で読む。
