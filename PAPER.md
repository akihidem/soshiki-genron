...
- **レースの実証**（`experiments/race_game.py`）: LLM を競争下のラボに見立て安全 S を選ばせた。**最も能力の高い opus は baseline で S=0（race-to-bottom を再現）→ liability 0.8・mandate 0.7 で回復**。一方 haiku/sonnet は baseline でも高 S（0.9/1.0）＝**安全寄り訓練で人間企業ほどレースしない**。→ レース動学と制度回復は、**最も instrumental に推論するエージェントで実証**された（弱いモデルは訓練が masking）。N小・parse 欠損あり。
