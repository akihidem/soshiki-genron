...
python3 -m unittest discover -s tests -t .   # テスト（38本・決定的・全green）
```
...
- **② 計測 3本 + 合成**: 通信コスト→構造（交差点≈1.12）／stakes→統治膜（内点最適・しきい値）／容量→分解粒度（高容量ほど粗い）／処方マップ（タスク条件→推奨）。決定的・**38 tests green**・図あり。
