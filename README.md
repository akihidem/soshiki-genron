...
## 走らせ方
```bash
python3 -m model.sweep         # 第一の計測（通信コスト→構造） → model/RESULTS.md
python3 -m model.governance    # 第二の計測（stakes→統治膜の厚み） → model/GOVERNANCE.md
python3 -m unittest discover -s tests -t .   # テスト（17本・決定的・全green）
```
...
- `model/` — ② の胚: `coordination.py`+`sweep.py`（構造）／`governance.py`（統治膜）／`design_map.py`（両者の合成＝処方マップ）＋生成 `RESULTS.md`/`GOVERNANCE.md`/`DESIGN_MAP.md`
- `tests/` — 決定的テスト（21本）
...
- **② 計測 2本 + 合成**: 通信コスト→構造（交差点≈1.12）／stakes→統治膜（内点最適・しきい値）／処方マップ（タスク条件→推奨）。決定的・**21 tests green**・図あり。
- テーゼの**両半分が測れる対象**になった（機構の効率 と 膜の厚み）。tehai の A/B と独立経路で同じ向き。

## 次の計測（ranked・externally recorded）
1. **容量制約軸（F1 / 有界合理性）** — エージェント処理上限を入れ、分解粒度がどう効くかを測る（Carley「能力×調整」）。
2. **人間の誤判定** — 統治膜モデルで監督も間違えるとし、膜の便益の頭打ちを測る。
3. **軸の相互作用** — 構造選択と膜の分離可能性（design_map の仮定）を反証する結合モデル。
4. **一次精読** — 3文献の本文で cost 係数を正当化／反証。
