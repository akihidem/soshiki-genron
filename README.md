...
> ⚠️ これは [`tehai`](../tehai) の続きではない。tehai（管理された委譲層 / 人間組織体系を参考に実装）は、この問いを生んだ**一つの実装例・一つのデータ点**にすぎない。本研究は tehai を起点にせず、組織そのものから問い直す。tehai の A/B 実測（`tehai/experiments/`）は参考証拠として引く。
