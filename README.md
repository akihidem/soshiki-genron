...
python3 -m unittest discover -s tests -t .   # テスト（109本・決定的mock・全green）
```
...
- `tests/` — 決定的テスト（109本・全 mock・green）
...
- **② 実証 6軸**（実 LLM・無料枠＋local gemma）: 監督スケーリング（recall/precision 接地）／レース（中立で liability 有効）／構造 F3（flat 安く同等品質）／**市場3レジーム（能力差で market が Pareto 支配）**／Goodhart（損 0.217・指数は閾値的で同定不可）。決定的部分 **123 tests green**。
