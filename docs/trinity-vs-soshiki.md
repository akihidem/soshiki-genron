...
**頑健に立った**: ① **rich feedback は Goodhart-safe＝0/30 セル**。② rich-loop は *profile 内*で再現的（negabinary@7B 3/3・wildcard_plus@24B 3/3）。③ **calc は 7B/24B とも 0/3・122B で 3/3（trials=3 で穴埋め完了）**＝122B 級が要ることが再現確定。**全段 Goodhart events = 0/36 セル**（rich feedback 安全性は決定的）。運用記録= local-llm-lab `experiments/2026-06-23-config-mesh-escalation.md`。

### capstone（実測 2026-06-24）— 検証ルーティング*市場*が単一モデルを Pareto 支配
profile（非単調）の必然として、3-tier を安い→高いに投げ verifier 通過で止める市場を実モデルで導出（`../experiments/config_mesh_market.py`）。コスト重み 7B=1 / 24B=3 / 122B=10:
...
市場ルート: negabinary→7B(1) / calc3→7B(1) / wildcard_plus→24B(4) / calc→122B(14)。
→ **市場 4/4 を cost 20＝flat-122B(4/4)の半額**。単一はどれも最大 2/4 しか解けない（profile 非単調）。**§5 p\*=w/s の実モデル支配を実証**：市場は profile を *verifier で吸収*し最安 solver を引く。市場は negabinary/calc3 を 7B に回す＝**簡単タスクに重い 122B の長考税/truncation を払わない**点でも優位。

※flat-122B の negabinary/calc3 は下記 harness 限界で計測不能（7B が解く易問＝実質 4/4@40 前提）。市場の結論は無傷（市場は 122B に到達しない）。

### 今後の課題: harness の reasoning-model 対応（measurement integrity）
config_mesh は reasoning モデル（Qwen3.5-122B）を正しく測れない＝**要修正（今後）**:
- `_mlx_chat` は `message["content"]` を読むが、reasoning モデルは思考を `message["reasoning"]` に入れ content は思考完了後。max_tokens 内に思考が収まらないと `finish_reason=length` で **content 欠落→空→「誤答(0.0)」と silent に取り違える**（8192/32768/`/no_think` いずれも calc3 で思考が溢れた）。→ truncation を能力不足と誤計測する。
- 直す3点: **(a) 最優先＝`finish_reason=length` を検知し「測定不能」として誤答と分離**（計測の正しさ）／(b) reasoning の field・thinking budget を per-model 対応／(c) thinking を確実に切る手段（mlx_lm.server `chat_template_kwargs:{enable_thinking:false}` 等）を特定。
