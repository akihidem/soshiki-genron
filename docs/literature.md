...
> メモ: 2件とも本研究の最初の計測（[`first-measurement.md`](first-measurement.md)）と向きが一致。Marschak&Radner が c_comm 軸の数理的土台、Malone が「下げると市場化」の予測元。次は一次精読で cost 係数の正当化、または Carley 計算組織論（②の測り方）。
